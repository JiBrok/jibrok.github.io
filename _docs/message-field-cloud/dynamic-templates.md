...
For security and performance, the template engine has these limits:

- **Maximum iterations**: 10,000 per template
- **Execution timeout**: 800ms
- **Maximum depth**: 128 nested levels
- **No eval**: No arbitrary code execution
- **Safe parsing**: No access to dangerous JavaScript features

## Performance Tips

1. **Minimize loops** - Avoid nested loops when possible
2. **Cache values** - Use `{% set %}` to avoid repeated calculations
3. **Early exit** - Use `{% if %}` to skip unnecessary processing
4. **Limit data** - Use max issues setting to limit data size
5. **Simple templates** - Complex logic can slow rendering
...
1. Enter your template in the description field
2. Click **Preview** button
3. Select a test issue
4. See the rendered output
5. Check for errors
...
Templates can be used with different output formats:

### HTML Output

```html
<h3>Issue {{ issue.key }}</h3>
<p>Status: <strong>{{ issue.fields.status.name }}</strong></p>
<ul>
{% for link in linkedIssues %}
  <li>{{ link.key }}: {{ link.fields.summary }}</li>
{% endfor %}
</ul>
```
...
#### Linked Issues:
{% for link in linkedIssues %}
- [{{ link.key }}]({{ link.self }}): {{ link.fields.summary }}
{% endfor %}
```
...
Linked Issues:
{% for link in linkedIssues %}
- {{ link.key }}: {{ link.fields.summary }}
{% endfor %}
```

## Next Steps

- [Template Examples](/docs/user-docs/template-examples/) - Ready-to-use template examples
- [JQL Templates](/docs/user-docs/template-examples/#jql-examples) - Dynamic JQL queries
- [Field Reference](/docs/user-docs/field-reference/) - Complete field listing
...
- 📖 [Template Examples](/docs/user-docs/template-examples/) - Copy and paste examples
- 🎫 [Support Portal](https://jibrok.atlassian.net/servicedesk/customer/portals) - Get expert help
- 💬 [Community](https://community.atlassian.com/) - Ask questions

---

**Pro tip:** Start with simple templates and gradually add complexity. Use the preview feature to test as you go!
