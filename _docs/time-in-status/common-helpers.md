...
The fields "Time in status", "Timer", and "Stopwatch" contain the "complex" value of the field. It includes time spent, break times, counter status, start date, and other information. But when it comes to exports or integrations, you need some specific value - for example, the time spent. To facilitate obtaining these specific values, helper fields are needed.

Helper fields work as follows:
* The main field is created ("Time in status", "Timer", or "Stopwatch").
* A helper field of the required type for the main field is created. The types currently available are number, date, and statistic.
...
For example, you can create a basic stopwatch field, create a helper for it that will display the stopwatch time in hours, and connect the helper to a third-party plugin that can handle numeric values. 
